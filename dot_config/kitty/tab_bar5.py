...
BG = as_rgb(color_as_int(opts.color2))
...
def _draw_icon(screen: Screen, index: int, cells: list) -> int:
    if index != 1:
        return 0
    ...
    # screen.cursor.fg, screen.cursor.bg = fg, bg
    # screen.cursor.x = len(ICON_LEFT + SEPARATOR_SYMBOL)
    screen.cursor.fg = 0 
    screen.cursor.bg = 0
    total_length = 0
...
def draw_tab(
    ...
    icon_cells = [] 
    icon_cells.append((ACTIVE_BG, BAR_BG, SEPARATOR_SYMBOL_RIGHT))
    icon_cells.append((ACTIVE_BG, BAR_BG, app))
    _draw_icon(screen, index, icon_cells)
